...
function LoadCalendarScript(callback){
	function LoadFullCalendarScript(){
		if(!$.fn.fullCalendar){
			$.getScript('devoopsv2/plugins/fullcalendar/fullcalendar.js', callback);
		}
		else {
	...
	if (!$.fn.moment){
		$.getScript('devoopsv2/plugins/moment/moment.min.js', LoadFullCalendarScript);
	}
	else {
...
function LoadLeafletScript(callback){
	if (!$.fn.L){
		$.getScript('devoopsv2/plugins/leaflet/leaflet.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function LoadTimePickerScript(callback){
	if (!$.fn.timepicker){
		$.getScript('devoopsv2/plugins/jquery-ui-timepicker-addon/jquery-ui-timepicker-addon.min.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function LoadBootstrapValidatorScript(callback){
	if (!$.fn.bootstrapValidator){
		$.getScript('devoopsv2/plugins/bootstrapvalidator/bootstrapValidator.min.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function LoadSelect2Script(callback){
	if (!$.fn.select2){
		$.getScript('devoopsv2/plugins/select2/select2.min.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function LoadDataTablesScripts(callback){
	function LoadDatatables(){
		$.getScript('devoopsv2/plugins/datatables/jquery.dataTables.js', function(){
			$.getScript('devoopsv2/plugins/datatables/ZeroClipboard.js', function(){
				$.getScript('devoopsv2/plugins/datatables/TableTools.js', function(){
					$.getScript('devoopsv2/plugins/datatables/dataTables.bootstrap.js', callback);
				});
			});
		});
	}
...
function LoadFineUploader(callback){
	if (!$.fn.fineuploader){
		$.getScript('devoopsv2/plugins/fineuploader/jquery.fineuploader-5.0.5.min.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function LoadXChartScript(callback){
	function LoadXChart(){
		$.getScript('devoopsv2/plugins/xcharts/xcharts.min.js', callback);
	}
	function LoadD3Script(){
		if (!$.fn.d3){
			$.getScript('devoopsv2/plugins/d3/d3.min.js', LoadXChart)
		}
		else {
			LoadXChart();
		}
...
function LoadFlotScripts(callback){
	function LoadFlotScript(){
		$.getScript('devoopsv2/plugins/flot/jquery.flot.js', LoadFlotResizeScript);
	}
	function LoadFlotResizeScript(){
		$.getScript('devoopsv2/plugins/flot/jquery.flot.resize.js', LoadFlotTimeScript);
	}
	function LoadFlotTimeScript(){
		$.getScript('devoopsv2/plugins/flot/jquery.flot.time.js', callback);
	}
	if (!$.fn.flot){
		LoadFlotScript();
	}
...
function LoadMorrisScripts(callback){
	function LoadMorrisScript(){
		if(!$.fn.Morris){
			$.getScript('devoopsv2/plugins/morris/morris.min.js', callback);
		}
		else {
			if (callback && typeof(callback) === "function") {
				callback();
	...
	if (!$.fn.raphael){
		$.getScript('devoopsv2/plugins/raphael/raphael-min.js', LoadMorrisScript);
	}
	else {
		LoadMorrisScript();
	}
...
function LoadAmchartsScripts(callback){
	function LoadAmchartsScript(){
		$.getScript('devoopsv2/plugins/amcharts/amcharts.js', LoadFunnelScript);
	}
	function LoadFunnelScript(){
		$.getScript('devoopsv2/plugins/amcharts/funnel.js', LoadSerialScript);
	}
	function LoadSerialScript(){
		$.getScript('devoopsv2/plugins/amcharts/serial.js', LoadPieScript);
	}
	function LoadPieScript(){
		$.getScript('devoopsv2/plugins/amcharts/pie.js', callback);
	}
	if (!$.fn.AmCharts){
		LoadAmchartsScript();
	}
...
function LoadChartistScripts(callback){
	function LoadChartistScript(){
		$.getScript('devoopsv2/plugins/chartist/chartist.min.js', callback);
	}
	if (!$.fn.Chartist){
		LoadChartistScript();
	}
...
function LoadSpringyScripts(callback){
	function LoadSpringyScript(){
		$.getScript('devoopsv2/plugins/springy/springy.js', LoadSpringyUIScript);
	}
	function LoadSpringyUIScript(){
		$.getScript('devoopsv2/plugins/springy/springyui.js', callback);
	}
	if (!$.fn.Springy){
		LoadSpringyScript();
	}
...
function LoadFancyboxScript(callback){
	if (!$.fn.fancybox){
		$.getScript('devoopsv2/plugins/fancybox/jquery.fancybox.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function LoadKnobScripts(callback){
	if(!$.fn.knob){
		$.getScript('devoopsv2/plugins/jQuery-Knob/jquery.knob.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function LoadSparkLineScript(callback){
	if(!$.fn.sparkline){
		$.getScript('devoopsv2/plugins/sparkline/jquery.sparkline.min.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function SparkLineDrawBarGraph(elem, arr, color){
    ...
    //Start Add by Hans
	elem.bind('sparklineClick', function(ev) {
		console.log(ev);
...
function TestTable3(){
	$('#datatable-3').dataTable( {
		...
		"oTableTools": {
			"sSwfPath": "devoopsv2/plugins/datatables/copy_csv_xls_pdf.swf",
			"aButtons": [
				"copy",
				"print",
				{
...
function MorrisDashboard(){
	Morris.Line({
		element: 'stat-graph',
		data: [
			{"period": "2014-01", "一年期EP": 13.4, "三年期EP": 55.3, '一年期EC': 1.5, '三年期EC': 0.3, 'Buyer':11, 'Company': 4.9, 'Mac': 9.6 , 'Mobile':4},
			{"period": "2013-12", "一年期EP": 10, "三年期EP": 55.9, '一年期EC': 1.5, '三年期EC': 3.1, 'Buyer':11.6, 'Company': 4.8, 'Mac': 9.2 , 'Mobile':3.8},
			{"period": "2013-11", "一年期EP": 8.6, "三年期EP": 56.4, '一年期EC': 1.6, '三年期EC': 3.7, 'Buyer':11.7, 'Company': 4.8, 'Mac': 9.6 , 'Mobile':3.7},
			{"period": "2013-10", "一年期EP": 9.9, "三年期EP": 56.7, '一年期EC': 1.6, '三年期EC': 1.4, 'Buyer':12.4, 'Company': 4.9, 'Mac': 9.6 , 'Mobile':3.3},
			{"period": "2013-09", "一年期EP": 10.2, "三年期EP": 56.8, '一年期EC': 1.6, '三年期EC': 0.4, 'Buyer':13.5, 'Company': 4.8, 'Mac': 9.3 , 'Mobile':3.3},
			{"period": "2013-08", "一年期EP": 9.6, "三年期EP": 55.9, '一年期EC': 1.7, '三年期EC': 0.4, 'Buyer':14.7, 'Company': 5, 'Mac': 9.2 , 'Mobile':3.4},
			{"period": "2013-07", "一年期EP": 9, "三年期EP": 56.2, '一年期EC': 1.8, '三年期EC': 0.4, 'Buyer':15.8, 'Company': 4.9, 'Mac': 8.7 , 'Mobile':3.2},
			{"period": "2013-06", "一年期EP": 8.6, "三年期EP": 56.3, '一年期EC': 2, '三年期EC': 0.4, 'Buyer':15.4, 'Company': 4.9, 'Mac': 9.1 , 'Mobile':3.2},
			{"period": "2013-05", "一年期EP": 7.9, "三年期EP": 56.4, '一年期EC': 2.1, '三年期EC': 0.4, 'Buyer':15.7, 'Company': 4.9, 'Mac': 9.7 , 'Mobile':2.6},
			{"period": "2013-04", "一年期EP": 7.3, "三年期EP": 56.4, '一年期EC': 2.2, '三年期EC': 0.4, 'Buyer':16.4, 'Company': 4.8, 'Mac': 9.7 , 'Mobile':2.2},
			{"period": "2013-03", "一年期EP": 6.7, "三年期EP": 55.9, '一年期EC': 2.4, '三年期EC': 0.4, 'Buyer':17.6, 'Company': 4.7, 'Mac': 9.5 , 'Mobile':2.3},
			{"period": "2013-02", "一年期EP": 5.7, "三年期EP": 55.3, '一年期EC': 2.4, '三年期EC': 0.4, 'Buyer':19.1, 'Company': 4.8, 'Mac': 9.6 , 'Mobile':2.2},
			{"period": "2013-01", "一年期EP": 4.8, "三年期EP": 55.3, '一年期EC': 2.6, '三年期EC': 0.5, 'Buyer':19.9, 'Company': 4.8, 'Mac': 9.3 , 'Mobile':2.2}
		],
		xkey: 'period',
		ykeys: ['一年期EP', '三年期EP','一年期EC','三年期EC','Buyer', 'Company', 'Mac', 'Mobile'],
		labels: ['一年期EP', '三年期EP','一年期EC','三年期EC','Buyer', 'Company', 'Mac', 'Mobile']
	})
    //Start Add by Hans	
	.on('click', function(i, row){
		  console.log(i, row);
	...
	Morris.Donut({
		...
		data: [
			{value: 40, label: 'Meet in Taiwan', formatted: 'at least 40%' },
			{value: 25, label: 'Meet online', formatted: 'approx. 25%' },
			{value: 10, label: 'RFQ', formatted: 'approx. 10%' },
			{value: 5, label: 'Trading Post', formatted: 'at most 5%' },
			{value: 15, label: 'Contact', formatted: 'at most 15%' },
			{value: 5, label: 'Get Latest Price', formatted: 'at most 5%' }
		],
	...
	Morris.Donut({
		...
		data: [
				{value: 15, label: 'Meet in Taiwan', formatted: 'at least 15%' },
				{value: 30, label: 'Meet online', formatted: 'approx. 30%' },
				{value: 20, label: 'RFQ', formatted: 'approx. 20%' },
				{value: 15, label: 'Trading Post', formatted: 'at most 15%' },
				{value: 15, label: 'Contact', formatted: 'at most 15%' },
				{value: 5, label: 'Get Latest Price', formatted: 'at most 5%' }
		],
		formatter: function (x, data) { return data.formatted; }
	...
	Morris.Donut({
		...
		data: [
				{value: 30, label: 'Meet in Taiwan', formatted: 'at least 30%' },
				{value: 15, label: 'Meet online', formatted: 'approx. 15%' },
				{value: 15, label: 'RFQ', formatted: 'approx. 15%' },
				{value: 20, label: 'Trading Post', formatted: 'at most 20%' },
				{value: 5, label: 'Contact', formatted: 'at most 5%' },
				{value: 15, label: 'Get Latest Price', formatted: 'at most 15%' }
		],
		formatter: function (x, data) { return data.formatted; }
	})
    //Start Add by Hans	
...
function DrawKnobDashboard(){
	var srv_monitoring_selectors = [
		...
		$("#knob-srv-7"),$("#knob-srv-8"),$("#knob-srv-9"),
		$("#knob-srv-10"),$("#knob-srv-11"),$("#knob-srv-12")
	];
...
$(document).ready(function () {
	...
	if (ajax_url.length < 1) {
		ajax_url = 'devoopsv2/ajax/dashboard.html';
	}
	...
	$('#search').on('keydown', function(e){
		if (e.keyCode == 13){
			e.preventDefault();
			$('#content').removeClass('full-content');
			ajax_url = 'devoopsv2/ajax/page_search.html';
...
$(function() {
	...
	var productplace = document.getElementById("productplace");
	$("select#newselect").on("change", function() { 
		DrawKnobDashboard();
	} );
	var today = new Date();
	var yesterday =
...
function ChartType(Type) {
	ChartHead.innerHTML = Type + "報表";
	if (Type == "日") {
		DateType.innerHTML = "前" + Type + "流量";
	} else {
		DateType.innerHTML = "本" + Type + "流量";
	}
	var fakeDataArray = new Array();
	fakeDataArray = [ 
	                  {name : "news"}, {name : "bizexchange"}, {name : "infolink"}, 
	                  {name : "sitemap"}, {name : "about_us"}, {name : "LNISW"}, 
	                  {name : "RNLD"}, {name : "BCN"}, {name : "AWS"}, 
	                 ];
	var tbody = '';
	var irandom = '';
	var angleup = '<i class="fa fa-angle-up"></i>';
	var angledown = '<i class="fa fa-angle-down"></i>';
	for ( var i in fakeDataArray) {
		var fakeData = fakeDataArray[i];
		...
		var tr = '<tr>';
		tr += '<td class="m-ticker"><b><a href="#" onclick="overviewinfo('+"'"+fakeData.name+"'"+')">' + fakeData.name + '</a>';
		tr += '</b><span>' + 'taiwantrade.com' + '</span></td>';
		tr += '<td class="m-price">' + parseInt(100*Math.random()) + '</td>';
		tr += '<td class="m-change">' + irandom + ' ' + parseInt(10*Math.random()) + ' (' + parseInt(10*Math.random()) + '&#37;)' + '</td>';
		tr += '<td class="td-graph"></td>';
		tr += '</tr>';
		tbody += tr;
	}
	$('#ticker-table tbody').html(tbody);
	runsparkline();
}
function overviewinfo(name) {
	var div = '';
	var fakeDataArray = new Array();
	fakeDataArray = [ 
	                  {name : "American", click : "34554", guest : "7638", act : "69473", engin : "Google", website : "new"},
	                  {name : "Taiwan", click : "345354", guest : "17638", act : "94732", engin : "Google", website : "main"},
	                  {name : "China", click : "95365", guest : "6931", act : "4973", engin : "Yahoo", website : "about_us"},
	                  {name : "Japan", click : "3964", guest : "3678", act : "7415", engin : "Google", website : "new"},
	                  {name : "Korean", click : "47839", guest : "3822", act : "4733", engin : "Yahoo", website : "main"},
	                  {name : "Singapor", click : "52154", guest : "23138", act : "83473", engin : "Google", website : "about_us"}
	                 ];
		var subdiv = '<div class="row newone-list-message">';
		subdiv += '<div class="col-xs-1"><i class="fa fa-users"></i></div>';
		subdiv += '<div class="col-xs-2"><b>國家</b></div>';
		subdiv += '<div class="col-xs-2">點擊次數</div>';
		subdiv += '<div class="col-xs-2">訪客數(不含當日重複)</div>';
		subdiv += '<div class="col-xs-2">直接流量</div>';
		subdiv += '<div class="col-xs-1">搜尋引擎</div>';
		subdiv += '<div class="col-xs-2">網站</div>';
		subdiv += '</div>';
	for ( var i in fakeDataArray ) {
		var fakeData = fakeDataArray[i];
		subdiv += '<div class="row newone-list-message">';
		subdiv += '<div class="col-xs-1"><i class="fa fa-user"></i></div>';
		subdiv += '<div class="col-xs-2"><b>'+ fakeData.name +'</b></div>';
		subdiv += '<div class="col-xs-2">'+ fakeData.click +'</div>';
		subdiv += '<div class="col-xs-2">'+ fakeData.guest +'</div>';
		subdiv += '<div class="col-xs-2">'+ fakeData.act +'</div>';
		subdiv += '<div class="col-xs-1">'+ fakeData.engin +'</div>';
		subdiv += '<div class="col-xs-2">'+ fakeData.website +'</div>';
		subdiv += '</div>';
	}
	div += subdiv;
	$('#dashboard-netmap').html(div);
	$('div#dashboard_tabs').find('div[id^=dashboard]').each(function(){
		$(this).css('visibility', 'hidden').css('position', 'absolute');
	});
	$('#dashboard-netmap').css('visibility', 'visible').css('position', 'relative');
...
function mapping(name) {
	...
	var fakeDataArray = new Array();
	fakeDataArray = [ 
	                  {name : "American", all : "34554", sevenD : "7638", yestD : "69473"},
	                  {name : "Taiwan", all : "345354", sevenD : "17638", yestD : "94732"},
	                  {name : "China", all : "95365", sevenD : "6931", yestD : "4973"},
	                  {name : "Japan", all : "3964", sevenD : "3678", yestD : "7415"},
	                  {name : "Korean", all : "47839", sevenD : "3822", yestD : "4733"},
	                  {name : "Singapor", all : "52154", sevenD : "23138", yestD : "83473"},
	                  {name : "Albania", all : "69478", sevenD : "2156", yestD : "38642"},
	                  {name : "Brazil", all : "24562", sevenD : "123463", yestD : "34683"},
	                  {name : "Chile", all : "265234", sevenD : "13564", yestD : "9355"},
	                  {name : "Dominica", all : "14634", sevenD : "7845", yestD : "2578"},
	                  {name : "Egypt", all : "47839", sevenD : "4678", yestD : "3457"},
	                  {name : "Germany", all : "36125", sevenD : "3485", yestD : "9645"}
	                 ];
	var subdiv = '<div style="margin-left:15px;margin-right:15px;margin-bottom:15px;">';
	subdiv += '<h4 class="page-header">商機配對 － ' + name + '</h4></div>';
	for ( var i in fakeDataArray ) {
		var fakeData = fakeDataArray[i];
		var srvNo = parseInt(i)+13;
		subdiv += '<div class="col-xs-12 col-sm-6 col-md-4">';
		subdiv += '<h4 class="newpage-header">'+ fakeData.name +'</h4>';
		subdiv += '<div class="row ow-server-bottom"><div class="col-sm-4">';
		subdiv += '<div class="knob-slider" style="pointer-events:none;">';
		subdiv += '<input id="knob-srv-' + srvNo + '" class="knob" data-width="60" ';  
		subdiv += 'data-height="60" data-angleOffset="180" ';
		subdiv += 'data-fgColor="#6AA6D6" data-skin="tron" data-thickness=".2" value="">';
		subdiv += '</div></div><div class="col-sm-8">';
		subdiv += '<div class="newrow"><i class="fa fa-windows"></i> 目前商機總數<b>' + fakeData.all + '</b></div>';
		subdiv += '<div class="newrow"><i class="fa fa-user"></i> 近七天新增數<b>' + fakeData.sevenD + '</b></div>';
		subdiv += '<div class="newrow"><i class="fa fa-bolt"></i>&nbsp;&nbsp;昨日新增數<b>' + fakeData.sevenD + '</b></div>';
		subdiv += '</div></div></div>';
	}
	div += subdiv;
	$('#dashboard-mapping').html(div);
	DrawMappingKnobDashboard();
	$('div#dashboard_tabs').find('div[id^=dashboard]').each(function(){
		$(this).css('visibility', 'hidden').css('position', 'absolute');
	});
	$('#dashboard-mapping').css('visibility', 'visible').css('position', 'relative');
}
function DrawMappingKnobDashboard(){
	var srv_monitoring_selectors = [
		$("#knob-srv-13"),$("#knob-srv-14"),$("#knob-srv-15"),
		$("#knob-srv-16"),$("#knob-srv-17"),$("#knob-srv-18"),
		$("#knob-srv-19"),$("#knob-srv-20"),$("#knob-srv-21"),
		$("#knob-srv-22"),$("#knob-srv-23"),$("#knob-srv-24")
	];
//將圖畫好
	srv_monitoring_selectors.forEach(DrawKnob);
//傳值並重新繪圖
...
function productType(name){
	var div = '';
	if (name == "日") {
		productplace.innerHTML = "前" + name + "統計型錄";
	} else {
		productplace.innerHTML = "本" + name + "統計型錄";
	}
	var fakeDataArray = new Array();
	fakeDataArray = [ 
	                  {name : "news", click : "34554", guest : "7638", act : "69473", engin : "24572"},
	                  {name : "bizexchange", click : "345354", guest : "17638", act : "94732", engin : "5894"},
	                  {name : "infolink", click : "95365", guest : "6931", act : "4973", engin : "35853"},
	                  {name : "sitemap", click : "3964", guest : "3678", act : "7415", engin : "53683"},
	                  {name : "about_us", click : "47839", guest : "3822", act : "4733", engin : "24578"},
	                  {name : "LNISW", click : "52154", guest : "23138", act : "83473", engin : "85732"}
	                 ];
		var subdiv = '<div class="row newone-list-message">';
		subdiv += '<div class="col-xs-1"></div>';
		subdiv += '<div class="col-xs-2"><b>網別名稱</b></div>';
		subdiv += '<div class="col-xs-2">新增型錄數</div>';
		subdiv += '<div class="col-xs-2">異動型錄數</div>';
		subdiv += '<div class="col-xs-2">下架型錄數</div>';
		subdiv += '<div class="col-xs-2">禁用型錄數</div>';
		subdiv += '</div>';
	for ( var i in fakeDataArray ) {
		var fakeData = fakeDataArray[i];
		subdiv += '<div class="row newone-list-message">';
		...
		subdiv += '<div class="col-xs-2">'+ fakeData.guest +'</div>';
		subdiv += '<div class="col-xs-2">'+ fakeData.act +'</div>';
		subdiv += '<div class="col-xs-2">'+ fakeData.engin +'</div>';
		subdiv += '</div>';
	}
	div += subdiv;
	$('#replaceMe').html(div);
	...
	$('#dashboard-planning').css('visibility', 'visible').css('position', 'relative');
}
function bussiness(label){
	console.log(label);
	$('#newselect').val(label);
	DrawKnobDashboard();
	$('div#dashboard_tabs').find('div[id^=dashboard]').each(function(){
	...
	$('#dashboard-servers').css('visibility', 'visible').css('position', 'relative');
}